...
{ // this object will change itself a little bit if it can be cached - then the Object refers instead to something that exists in the level. 
...
[System.Serializable]
public class BindExternalFunction
{ // the event this calls needs to have an object[] array as its 'dynamic' entry point
...
public class TagListener : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (selfInitialize && InkWriter.main != null)
        { // self-initializing listeners should only be placed on objects that are guaranteed to spawn in later than load...
    ...
    public void TagListenerFunction(string tag)
    {
        if (gameObject.activeSelf && enabled)
        {
            ...
            foreach (CustomTagEvent cte in customEvents)
            { // lets you make some simple custom events for tags for testing etc
